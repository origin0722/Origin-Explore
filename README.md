...
- **纯前端 + 本地存储**：所有数据仅保存在本机（网页版存浏览器 localStorage；桌面版存 `%APPDATA%\OriginExplore` 数据文件，重启/关机后稳定保留），无后端、无账号、无订阅
- **自带密钥（BYOK）**：在设置中添加任意 OpenAI 兼容接口的模型（DeepSeek / OpenAI / Claude / Gemini / Qwen / GLM / Kimi / MiniMax 等 11 个预设），密钥仅存本机，请求由浏览器直连你的 API 地址
- **双形态**：Web（Next.js 16）+ 桌面（Electron 安装版）
...
| 🧠 个人记忆 | 设置中集中管理「关于我」的事实（职业/兴趣/背景），所有对话与卡内问答自动带上这些背景 |
| 🔎 联网搜索 | 不确定的问题先实时检索网页再回答，附来源链接 |
| 🖼 视觉模式 | 发送图片让 AI 看图：多模态主模型直传原图，纯文本主模型由视觉模型识图（支持粘贴/拖拽，同图缓存） |
| 📦 数据备份 | 一键导出/导入完整备份（项目+思维宇宙+文档+设置，可选包含 API 密钥），按 id 合并还原 |
...
Windows 桌面版把 Next.js standalone 服务器与 UI 一起打包进单个安装包（Electron + utilityProcess 运行，**朋友机器无需安装 Node**）：

- **安装版**（`OriginExplore-<version>-setup.exe`）：NSIS 安装器，支持自定义安装目录与桌面快捷方式，安装后启动更快（零解压）
- 便携版已废弃，不再产出。

> 桌面版与网页版是同一套 UI；数据都存本机（桌面版在 `%APPDATA%\OriginExplore` 下，网页版在浏览器 localStorage，互不互通）。桌面版端口固定复用（首次选定后记入 `userData/explore-port.json`），重启/关机后 API 配置与对话数据稳定保留。
...
```bash
npm run package:app  # 生成 release/OriginExplore-<version>-setup.exe（NSIS 安装版）
```
...
src/
  ...
  components/sites/ai-explore-poker-820d0558/
    app-context.tsx        # 全局状态中心（项目/对话/文档/思维宇宙/设置；桌面版文件持久化，浏览器 localStorage）
    shell.tsx              # 主布局（侧边栏 + 对话区 + 思维宇宙面板 + 弹窗）
    sidebar.tsx            # 侧边栏（项目/文件夹/本地文档/收藏/使用文档/设置/账户/GitHub 入口）
    chat-card.tsx          # 对话卡片（术语树递归深挖 + 发散/分支 + 平行视图）
    input-area.tsx         # 输入区（模型选择/联网开关/引用/三态发送）
    modals.tsx             # 设置（含 BYOK 测试连接/编辑）/ 新手引导 / 使用文档 / 账户
  ...
  lib/sites/ai-explore-poker-820d0558/
    ...
    term-detect.ts         # 术语检测
    vision.ts              # 视觉路由/图片降采样/识别缓存
  types/                   # 共享类型
electron/
  main.js                  # Electron 主进程（GPU 策略 + 崩溃兜底 + 本地服务器 + 数据文件读写）
  preload.js               # 预加载桥接（app-info / 数据持久化 / 打开数据目录，最小暴露面）
  loading.html             # 启动加载页
...
v1.0.1（2026-08-17）：
- 术语卡片对话持久化（刷新/切视图不丢）、线程级输入锁（主流式期间发散/分支卡可并行输入）、设置「关于」版本检查
- 视觉模式（Vision Mode）——发送图片让 AI 看图（多模态直传 / 视觉模型路由识图 / 同图缓存）
- 个人记忆——设置中集中管理「关于我」的事实，所有对话自动带上背景
- 侧边栏新增 GitHub 仓库入口
- 收藏功能链路完整确认（旧安装包问题，重打包解决）（首个正式版）
