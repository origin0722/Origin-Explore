...
import type { ModelTier } from "@/types/sites/ai-explore-poker-820d0558";
...
const TIER_BADGE: Record<ModelTier, string> = {
  free: "border-std text-text-tertiary",
  pro: "border-brand/30 bg-brand/10 text-brand",
  max: "border-brand/40 bg-brand/15 text-brandtw",
};

export function InputArea() {
  const { settings, setSettings, sendMessage, busy, setActiveDocId } = useApp();
  ...
  const activeModel = MODELS.find((m) => m.id === settings.activeModelId);
  ...
  return (
    <div className="w-full flex justify-center px-2 pb-[max(0.5rem,env(safe-area-inset-bottom,0px))] sm:px-4 sm:pb-0">
      <div
        ref={rootRef}
        className={`relative bg-inputarea shadow-card border-2 border-std rounded-[28px] p-2.5 gap-0.5 flex flex-col w-full max-w-[900px] transition-colors focus-within:border-brand/50 ${
        ...
        <div className="flex items-end gap-2">
          ...
          <div className="relative flex-shrink-0">
            <button
              ...
              className="flex items-center justify-between gap-1 text-sm bg-btn-selector shadow-selector rounded-full px-3 h-8 min-w-0 max-w-[160px] sm:max-w-[220px]"
            >
                ...
                {MODELS.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`flex-shrink-0 rounded border px-1.5 text-[10px] leading-4 ${TIER_BADGE[m.tier]}`}
                      >
                        {TIER_LABEL[m.tier]}
                      </span>
          ...
          <button
            ...
            aria-label="本地文档"
            title="本地文档"
            className="h-8 w-8 sm:h-[34px] sm:w-[34px] flex-shrink-0 rounded-full bg-btn-inputarea-transparent-hover hover:bg-btn-inputarea-transparent-hover/20 flex items-center justify-center text-text-icon-secondary transition-colors"
          >
            <Paperclip size={16} />
          </button>
          ...
          <textarea
            ...
            className="block w-full min-h-0 flex-1 bg-transparent scrollbar-inputarea text-primary text-base leading-6 resize-none outline-none placeholder:text-text-quaternary max-h-[192px] overflow-y-auto py-0.5"
          />
        </div>
