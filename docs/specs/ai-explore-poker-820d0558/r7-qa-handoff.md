...
> 本文件记录 R7 阶段全部关键发现与未完成任务，供新 agent 无缝接手。
> 双基准：1440×900（桌面）+ 390×844（移动），diff 原站截图，见 `scripts/qa-pixel.mjs`。
> 运行中的 dev server: `npm run dev`（localhost:3000，Turbopack 热更新正常）。
