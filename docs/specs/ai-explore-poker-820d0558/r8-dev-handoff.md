...
## 六·五、阶段总结（2026-08-14 收尾快照）

**R8 完整成果**：从"纯 UI 克隆"演进为可交互的个人知识探索工具（mock 数据 + BYOK 双通道、数据仅存本机）：

- **对话核心**：轮次制对话（分支/引用/自动标题）、术语卡片=可对话卡片（↗️子/➡️关联/⬇️分支 + 卡内继续追问 + 深挖路径面包屑）、每轮探索路径块（记录点开的词条链条）。
- **知识管理**：轮次收藏区 + 智能摘要（BYOK 流式/离线启发式）、思维宇宙 3D（仅真实 parentSubject 连线 + 连接链）、卡片树导航（轮次+术语卡片同图、点击跳转/重开、右键未读）。
- **双通道 AI**：BYOK（OpenAI 兼容 SSE 流式 + 15s 首字超时 + 优雅回退）/ 离线知识库（TERM_TREE 8 根 4 层 + GLOSSARY 60 条 + 上下文记忆）。
- **文档**：导入（pdf/docx/md/txt/html）→ 划词即问 → 问答进对话。
- **借鉴原站介绍**：使用指南弹窗、术语下划线智能标注、引用回答、分支继承上游历史、未读指示、收藏智能摘要。
- **布局（多轮打磨终稿）**：悬浮侧边栏 + 对话框页面居中（空态居中略偏右）→ 出现卡片树时左缘不动、右缘平滑回收（padding 过渡 + 树淡入）→ 卡片树 → 思维宇宙贴最右。
- **测试体系**：22 个 `verify-*.mjs`（puppeteer + Edge），覆盖交互、BYOK 拦截、GL 像素分析、布局断言；`tsc` + `npm run build` 通过。

**待续方向**：移动端适配（用户明示最后做）、文档划词浮条跟随选区、卡片"返回/分支"中间态动画、像素 diff 收尾。
