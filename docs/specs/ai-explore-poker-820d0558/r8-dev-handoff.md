...
66. **导航改为卡片树列表 + 放大**（用户反馈"不一定是有向图/图太小"）：`TurnGraph` 从 SVG 球点图改为**树状列表**（原站卡片树式）：轮次根行（○ + 标题 + ⭐ + 未读点）按时间纵排、分支轮次（parentTurnId）以 ⬇️ 前缀嵌套在来源轮次下、术语卡片按探索链条缩进（↗️/➡️/⬇️ 图标 + 术语名）并带左侧引导线；文字常显、面板加宽 232→280px（聊天右留白 248→296px），保持无框架透明浮层 + 居中偏下。
67. **贴底跟随修复（滚动 bug）**：自动滚底原用"瞬时距离判断"——内容未溢出时 scrollTop 恒 0，恰好溢出那一刻判断翻转为 false，导致回复变长后不再跟随（用户看不到新内容，还误触发未读）。改为 **stickToBottom 引用**：贴底（含无溢出）时跟随滚底，scroll 事件里用户上滚才解除、滚回底部恢复；自动未读误报同步消失。测试：fav-unread 全流程回归通过。
68. **卡片树移入独立右侧区域 + 图标规整**（2026-08-14，用户反馈"右侧空/对话框窄/子卡片遮挡/emoji 不美观"）：shell 主区改为 flex 行——对话区（flex-1，聊天卡片居中最大 990px）+ **轮次导航卡片树独立列（w-[300px]，位于对话框与思维宇宙之间）**（`my-auto pt-[8vh] max-h-[62%]` 居中偏下）；ChatCard 内浮动面板与右留白移除——子卡片级联不再遮盖卡片树、对话框恢复全宽；树的 ↗️/➡️/⬇️ emoji 换成 lucide 图标（ArrowUpRight/ArrowRight/ArrowDown，11px、strokeWidth 2.4，分支轮次前缀同步换 ArrowDown）。
69. **四段式布局规范**（2026-08-14，用户反馈"滚动条不在最右/思维宇宙插在中间/对话框偏左拥挤"）：侧边栏桌面端从 overlay 改为**真实占位列**（`sm:static`，对话框不再被压、发消息后折叠 56px 也正常让位）；主区固定顺序 **对话区 → 轮次卡片树（mr-5 让出折叠条）→ 思维宇宙折叠条贴页面最右**（mindscape 面板/rail 从对话区内移回 main 级 right-0）；FAB 与 ICP 脚注对齐对话框底部中央。`verify-ui-fixes` 新增 1c/1d/1e 布局断言（rail 右缘=1440、三段顺序、对话框不压侧边栏）。
70. **三段同步滑动**（2026-08-14，用户反馈"侧边栏与对话框动、卡片树不动很突兀"）：卡片树改为**锚定对话框右缘**（对话框容器 `relative` + 树 `absolute left-full ml-5 w-[380px] pr-5`）；对话列右侧预留 400px（`lg:pr-[400px]`），侧边栏折叠时预留改为 **484.5px**（释放宽度 169px 的一半 84.5px 留在右侧、另一半给对话框），`transition-[padding-right] duration-200` 与侧边栏 200ms 动画同步——**侧边栏、对话框右缘、卡片树三者一起向左平滑滑动**，对话框在剩余空间内尽可能居中（宽屏时整组居中）。
71. **页面居中固定（不再跟随侧边栏）**（2026-08-14，用户最终取舍"跟随移动太偏左"）：侧边栏回到**悬浮式 overlay**（桌面端不占流、不挤压内容），main 固定 `lg:pl-[225px]` 让出侧边栏；对话框（居中最大 990）+ 卡片树（锚定右缘 380）整体**固定在页面居中位置**——侧边栏展开/折叠时两者**不动**，折叠后左侧自然留白。`verify-ui-fixes` 断言：cardLeft 恒 225（不随折叠变化）、rail 贴最右、三段顺序保持。
72. **空态居中 + 出现卡片树右缘回收动画**（2026-08-14，用户方案）：无轮次时对话框按**页面居中（略偏右 16px）**展开——左缘 = `max(0, (100vw - min(990px,100vw))/2 - 209px)` 一次算好并固定（`data-dialog-root`）；开始对话后对话列 `lg:pr-[400px]`（`transition-[padding-right] duration-300`）让出卡片树位——**对话框左缘不动、右缘平滑回收**（990→815），卡片树（锚定右缘）随回收淡入浮现（`fadeIn 300ms`）。`verify-ui-fixes` 新增 0b（空态中心≈736）与 1f（出现树后 left 不变、right 收缩）。
